...
function tableForCases(cases) {
  ...
  return `<div class="table-wrap"><table><thead><tr><th>样本与要求</th><th>原始音频</th>${candidateHeaders}</tr></thead><tbody>${cases.map(item => caseRow(item, candidateCount)).join("")}</tbody></table></div>`;
}
